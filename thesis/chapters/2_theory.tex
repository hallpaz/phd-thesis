...
The \textbf{Fourier transform} is the mathematical operation that connects the time or space domain with the frequency domain. It decomposes a signal into a set of sine and cosine waves, each with a specific frequency, amplitude, and phase. For a continuous-time signal \( f(t) \), the Fourier transform \( F(f) \) is defined as:

\begin{align}
  F(f)(s) = \hat{f}(s) = \int_{-\infty}^{\infty} f(t) e^{-2\pi i t s} dt
\end{align}
where:

\begin{itemize}
  \item \( f(t) \) is the time-domain signal.
  \item \( F(f)(s) \) is the frequency-domain representation.
...
The Fourier transform has multiple forms depending on the nature of the signal. The \textit{continuous Fourier transform} (CFT) is used for continuous-time signals, decomposing them into their frequency components. In contrast, the \textit{discrete Fourier transform} (DFT) applies to discrete or sampled signals, enabling frequency-domain analysis in digital systems. Computing the DFT directly is computationally expensive, but the \textit{fast Fourier transform} (FFT) provides an efficient algorithm that significantly reduces complexity, making real-time signal processing feasible in practical applications \citep{oppenheim-schafer2009}.
...
When signals are sampled, we effectively multiply the original continuous signal by a sequence of Dirac delta functions (impulses). In the frequency domain, this multiplication corresponds to a convolution with a train of impulses, which leads to periodic replicas of the signal's frequency spectrum. This relationship emphasizes the importance of the Nyquist criterion: if the sampling rate is too low (below twice the highest frequency in the signal), these spectral replicas overlap, resulting in aliasing. For more informaion on signal processing fundamentals, please, refer to \cite{diniz2014processamento}, \cite{oppenheim-signals} and \cite{oppenheim-schafer2009}.
...
Multiresolution analysis (MRA) offers a hierarchical approach to understanding signals at different levels of detail. By decomposing signals into components that represent varying scales of resolution, MRA enables the simultaneous capture of both global trends and fine details. This framework, formally introduced by \cite{mallat1989theory}, is particularly powerful in signal processing and computer vision, as it aligns with the way we naturally perceive visual information—focusing on broader structures while also resolving intricate details when needed \citep{marr82}.
...
One of the simplest and most common techniques in MRA is the use of \textit{Gaussian and Laplacian pyramids}. These pyramids decompose signals into a series of increasingly coarse approximations, representing the signal at multiple resolutions. This multiscale representation enables efficient manipulation and analysis across different levels of detail. The concept of pyramidal representations was introduced by \cite{Burt1981FastFT} and further developed by \cite{burt1983laplacian}, who demonstrated their effectiveness in image processing tasks such as compression and multi-scale analysis.
...
For more details on wavelets and multiresolution analysis, please refer to \cite{mallat1989theory}, \cite{daubechies92} and \cite{GomesVelho2015FromFA}.
...
Neural networks learn through a process called \textbf{backpropagation} \citep{Rumelhart1986}, which efficiently computes gradients needed for optimization. The learning, or training, process involves two main steps: 
...
Perceptrons can be connected in a variety of ways, defining different neural networks architectures. A \textbf{Multilayer Perceptron} (MLP) is a class of artificial neural networks composed of multiple layers of perceptrons, where each layer is fully connected to the next \citep{Rumelhart1986MLP}. An MLP typically consists of an input layer, one or more hidden layers, and an output layer. Each layer applies a series of weighted transformations to its inputs, followed by a non-linear activation function.
