...
\subsubsection{M-Net: Hierarchical Modulated Stages}
\label{s-mnet}
...
Tre training of the MR-Net $f$ aligns closely with the hierarchical nature of the network's architecture, where each stage $g_i$ is tasked with learning a specific level of detail in the signal $\gt{f}$. 

In a typical MR-Net training regimen, the process unfolds sequentially, beginning with the lowest frequency stage and progressively moving towards higher frequency stages. This stage-wise learning strategy allows the model to construct a comprehensive representation of the signal, starting from coarse details and gradually incorporating finer nuances. To maintain the integrity of the multiresolution representation, MR-Net training often employs a progressive learning approach. Once a stage is trained to capture a certain level of detail, its weights are typically "frozen." This strategy ensures that the details are added incrementally from coarse to fine, preserving the hierarchical nature of the learned representation.

The training process offers flexibility in terms of input processing. It can work directly with the original signal or utilize a pre-processed version using low-pass filters, accommodating different multiresolution schemes. This adaptability makes the MR-Net framework suitable for a wide range of signal types and applications.
...
% ddddddd
% The training process for a Multiresolution Network (MR-Net) is a sophisticated approach designed to capture the intricate details of a signal across various scales. At its core, MR-Net training aligns closely with the hierarchical nature of the network's architecture, where each stage is tasked with learning a specific level of detail in the signal.
...
While the reconstruction of signals using the Gaussian tower is perfect, it is also wasteful if 
