...
Multiresolution analysis has its roots in the seminal work of (\cite{mallat1989theory}) and (\cite{daubechies92}), who formalized the wavelet transform as a powerful tool for analyzing signals at different scales. Their work laid the groundwork for decomposing signals into various frequency bands while preserving temporal information, a crucial aspect in many signal processing applications.

The integration of multiresolution concepts with neural network architectures has led to several innovative approaches. \cite{bruna2013} proposed scattering networks, which use cascaded wavelet transforms followed by nonlinear operations to create stable, translation-invariant representations of signals. Works such as those by Ke et al. (2017) \red{FIND CITATION} have explored CNN architectures that process inputs at multiple scales simultaneously, mimicking the multi-scale nature of wavelet transforms. \red{However, these models architectures are data-networks designed to extract features from large datasets to solve problems like image classification, and not represent signals.}
% Multi-Scale CNNs: 

\cite{zhangWavelet92} incorporate wavelet functions as activation functions and propose Wavelents Neural Networks (WNNs) as an alternative architecture to traditional artificial neural networks. A WNN uses a wavelon instead of a perceptron as basic computational unit to perform a kind of adaptive wavelet transform on the input signal. Unlike deep neural networks that learn hierarchical features, WNNs typically have a shallow architecture, what impacts its representational capacity, requiring much more parameters to create high-fidelity representations of signals. Also, WNNs can be challenging to optimize and their  performance can be highly dependent on the initial choice of wavelet functions and their parameters.

In terms of coordinate-based neural networks, \cite{bacon2021} introduced the Band-Limited Coordinate Network (BACON), a Multiplicative Filter Network (\cite{fathony2020multiplicative}) that produces intermediate outputs with an analytically specified spectral bandwidth, achieving multiresolution representations of underlying signals. While its structure allows BACON to be expressed as a linear combinations of sines, avoiding the composition of sines present in deep sinusoidal MLPs, it creates multiresolution representations by truncating the frequency spectra of the signals at a specific value. This approach produces ringing artifacts in some levels of detail, and becomes evident when we look at the Fourier transform of images reconstructed using this network (see Chapter \ref{ch:imaging}).
