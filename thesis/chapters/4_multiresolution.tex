...
Our goal is to design a new architecture of neural networks capable of encoding signals at multiple scales. We aim for a continuous and compact representation of scalar fields that can be used to model images, shapes, and other complex objects either as an explicit attribute of space-time coordinates or as level sets of implicit functions.

In this chapter, we present hypotheses, experiments, and discussions related to the representation of one-dimensional signals using sinusoidal neural networks. By initially working with one-dimensional signals, we can better control the signal frequencies, easily visualize them by plotting their Fast Fourier Transform (FFT), and validate the results in terms of classical sampling theory.

We build our architecture upon SIREN (Sinusoidal Representation Networks) \cite{sitzmann2019siren}, which is capable of learning high-frequency details of a signal quickly and is also suitable for modeling the signal derivatives. To do this, we explore the initialization of the network by better understanding the relationship between its hyperparameters, the frequencies of a signal, and the frequencies learned by the network.

For the multiresolution approach, we leverage classical structures of image and signal processing, such as Gaussian and Laplacian Pyramids. We develop a framework to train a multi-stage neural network using multiresolution structures in an efficient way. We demonstrate that our model can maintain a comparable reconstruction quality to a simple SIREN model with the same number of parameters while encoding multiple levels of detail.
