...
% Repeating this experiment with more neurons per layer, we observe that we can use higher values of $\omage_0$ to correctly learn to represent the input signal (Figure \ref{}).
...
By comparing the Fast Fourier Transform of the learned signal and the target signal (Figure XXXXX), we conclude that the network learned the low frequencies of the signal up to an unknown value that increases or decreases with $\omega_0$.
