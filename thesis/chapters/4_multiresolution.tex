...
Moreover, the authors of SIREN propose to initialize the first layer of the network so that the sine function $\sin(\omega_0 \cdot W x + b)$ spans multiple periods over $[-1, 1]$. They introduce $\omega_0$ as a hyperparameter that could be adjusted to each signal. For the examples presented in their work, they used a fixed $\omega_0=30$ and found it to work well empirically, but they did not dive deeper on the why.
...
By understanding and adjusting \(\omega_0\) and \(\omega_h\), we aim to optimize the training process of sinusoidal neural networks and develop a multiresolution architecture with them.
...
We first tried to recover the lower frequencies using $\omega_0=10$. The reconstructed signal (Figure \ref{fig:rec-naive-w0}) resembles a very low-frequency signal. In fact, the Fast Fourier Transform of both signals (Figure \ref{fig:fft-smooth-4freqs}) reveals that the network only captured a peak at 2 Hz and learned only small amplitudes of other low frequencies, representing a heavily smoothed version of the input signal.
...
\begin{figure}[h!]
    ...
    \caption{Reconstruction of a signal with low and high frequencies using a network initialized with very low frequencies.}
\end{figure}


We believe this result is due to the network being initialized with frequencies lower than those present in the signal. Given that the sine function is periodic with a period of $2\pi$, we propose that the hyperparameter $\omega_0$ should always be multiplied by $2\pi$ to better establish a relationship between the initialization of the first layer and the range of frequencies represented by each atom in the network's first layer.

Repeating the experiment with $\omega_0 = 10$Hz, we observe the expected behavior: the predicted wave (Figure \ref{fig:rec-2pi-w0}) approximates the low-frequency portion of the signal. Notice how the FFT plot (Figure \ref{fig:fft-low-4freqs}) matches the peaks at 2 Hz and 5Hz, while the high frequency tones are not learned.
...
\begin{figure}[h!]
    ...
    \caption{Reconstruction of a signal with low and high frequencies using a network initialized with frequencies higher than the lowest present in the signal.}
\end{figure}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \caption{Reconstruction with $\omega_0 \in [25, 45]$}
        \label{fig:rec-25-45}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \caption{FFT with $\omega_0 \in [25, 45]$}
        \label{fig:fft-25-45}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/ch4/prediction_w35-45.pdf}
        \caption{Reconstruction with $\omega_0 \in [35, 45]$}
        \label{fig:rec-35-45}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \caption{FFT with $\omega_0 \in [35, 45]$}
        \label{fig:fft-35-45}
    \end{subfigure}
...
These experiments validate our hypothesis that a shallow network with only one layer of sinusoidal activation functions can filter a signal by band-limiting its frequency content, providing a spectral filter similar to those in MFN-based architectures. However, when we add a hidden sinusoidal layer to the network, it becomes much more challenging to isolate specific frequencies. Figure \ref{f:w10-1hl-64hf} shows the reconstruction of a sinusoidal network with one hidden layer and 64 neurons per layer, initialized with $\omega_0=10$ Hz. Like in the experiment with a shallow network with 256 neurons, that was initialized with a broader spectrum of frequencies, the reconstruction seems to perfectly fit the input signal both in spatial and spectral domains. Moreover, Figures \ref{fig:4x-freqs-1hl-64hf} and \ref{fig:8x-freqs-1hl-64hf} present two zoomed views for a more detailed analysis. For this visualization, the network is evaluated in a grid with 512 samples within a much smaller interval, so most samples were not part of the training data. Notice that the reconstruction is smooth, with no spurious noise between the supervised samples.
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Reconstruction with 1 hidden layer and 64 neurons per layer}
    \label{f:w10-1hl-64hf}
\end{figure}

Although the width of the network was kept constant as 64 neurons, the addition of a single hidden layer greatly increases the size of the model, and thus it's capacity, what could be the reason why it fits the signal better, even though it was initalized with lower frequencies. For instance, a shallow model with 64 neurons per layer has 192 parameters while a model with 1 hidden layer and 64 neurons per layer has 4352 parameters.
To better understand the impact of the hidden layer in this process, we ran other experiments decreasing the width of the networks with 1 hidden layers and increasing the with of those networks without hidden layers. For these experiments, all models were initialized with frequencies in $[-10*2\pi, 10*2\pi]$. Figure \ref*{fig:rec-1hl-16hf-w10} shows the reconstruction of a model trained with 1 hidden layer and 16 neurons per layer, with a total of 320 parameters. Note that the model still captures the high frequencies in the signal although some noise can be perceived when we look at its FFT (Figure \ref{fig:fft-1hl-16hf-w10}). However, the model with no hidden layers and 1024 neurons per layer cannot capture any of the high frequencies (Figures \ref{fig:rec-0hl-1024-w10hf} and \ref{fig:fft-0hl-1024-w10hf}), despite having 3072 parameters.
...
\begin{figure}[h!]
    ...
    \end{subfigure}
    \caption{Comparison of smaller models with a single hidden layer against a bigger model without hidden layers.}
    \label{f:hidden-layer-vs-no-hiddel-layer}
\end{figure}

A similar phenomenon is observed when the network is initialized with only the highest frequencies. We ran new experiments initilizing the networks with frequencies in $[-35*2\pi, -25*2\pi] \cup [25*2\pi, 35*2\pi]$. Figure \ref{fig:rec-1hl-16hf-35w45} shows that a network with 1 hidden layer and 16 neurons per layer can capture the lower frequencies, despite show some noise in its spectrum (Figure \ref{fig:fft-1hl-16hf-35w45}), while a network with no hidden layers and 1024 neurons per layer cannot capture the lower frequencies (Figures \ref{fig:rec-1hl-1024hf-35w45} and \ref{fig:fft-1hl-1024hf-35w45})
...
We conclude that a sinusoial neural network with no hidden layers can effectivelly filter and decompose the frequencies of a sinal. However, it seems that, in general, it not straightforward to decompose the frequencies of a signal using a sinusoidal deep network with at least one hidden layer.
...
We initialize a Siren network with the \red{following architecture}:
...
In the first experiment, we initialize a Perlin Noise with \red{PARAMETERS OF THE NOISE}. 
...
If we use a low $\omega_0$ such as $20$, our result looks like a smoothed version of the signal (Figure \ref{fig:rec-smooth-noise1}), as if we were reconstructing only its low frequencies. We plotted the Fast Fourier Transform (FFT) of both the original signal and its reconstruction in order to verify that the low frequencies match (Figure \ref{fig:fft-smooth-noise1}).} Note that the FFT plot presents a distribution of multiple frequencies up to 256. By comparing the Fast Fourier Transform of the learned signal and the target signal, we concluded that the network learns the low frequencies of the signal up to an unknown value that increases or decreases with $\omega_0$.
