...
% directly operate on the original signal or employ a pre-processed version filtered through a low-pass filter, supporting diverse multiresolution schemes.

% Depending on the desired frequency characteristics for each stage, the input to MR-Net can either be the original ground-truth signal or a low-pass filtered version, allowing the network to focus on different levels of granularity.
