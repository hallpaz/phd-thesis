...
% To ensure the integrity of the multiresolution representation, once a stage is trained to capture a particular level of detail, its weights are "frozen", preserving the learned coarse features as finer features are added incrementally.
...
There are two primary sampling strategies: \textit{regular} and \textit{irregular} sampling. In this work, we focus on regular sampling, also known as uniform sampling, which is more commonly used and is supported by a well-established theoretical foundation for sampling and reconstruction. \red{However, Chapter \ref{chap:future} will touch on some intriguing possibilities for irregular sampling and areas where it could offer future research opportunities. (remover se eu não escrever)}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.30\linewidth]{img/ch4/grayscale.png}
    \includegraphics[width=0.30\linewidth]{img/ch4/blurred.png}
    \includegraphics[width=0.30\linewidth]{img/ch4/dog.png}\\
...
% we opt to initialize $g_i$ such that it can represent frequencies up to $\omega_i:=\frac{r_i}{2}$.
...
% To ensure that $g_i$ can effectively represent the frequency range of $\gt{g}_i$, we initialize its parameters to handle frequencies up to $\omega_i = \frac{r_i}{2}$. We do this by uniformly sampling the rows of the first matrix $W_{s_i}$ in the set $\Omega_i = \left[-\omega_i, \omega_i\right]^2$. 


% In practice, the sinusoidal layer $S_i$ may reuse some of the frequencies initialized in previous stages, as the decomposition $\{\gt{g}_i\}$ follows a Laplacian pyramid, ensuring that $\Omega_0 \subset \cdots \subset \Omega_{N-1}$.
...
% The reconstruction of a signal involves a combination of interpolation and fitting. In our case, 
...
\red{Senti que poderia ter mais figuras neste capítulo, mas despriorizei em face do tempo. O que acha? É necessário pririzar alguma delas? Exemplos

\begin{itemize}
    \item Na 4.4: átomos resultantes das camadas escondidas de cada subclasse da MR-Net (meio que tem uma representação na seção MR-Module...).
    \item Na 4.5.2: Reconstrução de um sinal em multiresolução (meio que ficou por conta do capítulo 3).
    \item Na 4.6: Alguma representação de que é contínua em escala.
\end{itemize}
...
% where $W_{s_i}$ has rows sampled at the set $\Omega_i$ defined in Section~\ref{sec:frequency-initialization}. By increasing $m$, the dictionary of input frequencies is augmented. The hidden block $H_i$ consisting of $k$ sinusoidal layers, further enhances this list of frequencies.
