...
In traditional approaches, textures are represented as discrete digital tiles, often accompanied by methods to mitigate visible seams or discontinuities when the pattern repeat. In contrast, we propose to use our multiresolution neural representation to encode seamless periodic textures in a continuous, compact and fast to evaluate digital representation. Specifically, we extend deep sinusoidal neural networks to define \textit{periodic neural networks}. Inspired by the Fourier series, we constrain the representational space of our network to the space of periodic functions. The spatial structure arrangement of textures makes them a suitable and important practical use case for our method.
...
Using a sinusoidal neural network, \citet{bacon2021} showed that BACON can extrapolate periodic tiles using integer frequencies. Since BACON is analyticaly equivalent to a shallow network, it is clear that the representation can be made periodic by initializing the network with frequencies that are integer multiples of a period $P$. However, for deep sinusoidal MLP such as Siren~\citep{sitzmann2019siren} or MR-Net, this could not be true, since the composition of sines generates much more frequencies, as illustrated in Figure \ref{f:generated-frequencies}. We prove that deep sinusoidal networks preserve this property and show applications based on it.


\begin{figure}[h]
\centering
\includegraphics[width=0.84\linewidth]{img/ch6/generated_frequencies.png}
...
Throughout this section, we assume that the signal is a monochromatic image, though the results can easily be extended to multi-channel images or any signal with an explicit representation in its natural domain as a function \( \gt{f}: \mathbb{R}^n \to \mathbb{R}^m \).
...
For the network $f$ to be periodic with periods $(P_1,P_2)$ we can set the frequencies $\omega_{ij}=k_{ij}\frac{2\pi}{P_i}$, with $k_{ij}$ being integers. In Section~\ref{sec:integer-initialization}, we discuss a initialization strategy for choosing $k_{ij}$ using ideas from Fourier series.
...
Without loss of generality, consider the case where $f$ is a one-dimensional function, i.e. $f:\R\to\R$. Assuming the first layer is periodic with period \( P \), its frequencies are \( \omega_j = k_j \frac{2\pi}{P} \). Moreover, observe that if we prove that each sinusoidal neuron, expressed as in Equation \ref{eq:sinusoidal-neuron}, expands as a sum of harmonics with period $P$, then, the output of $h$ is periodic with period $P$. The proof is based on this identity presented in~\citet{novello2022understanding}:
...
\label{sec:integer-initialization}
...
We note that, using trigonometric identities, an input neuron can be rewritten as: 
\begin{align}
  \sin(\omega_i x + \varphi_i) = \cos(\varphi_i) \sin(\omega_i x) - \sin(\varphi_i)\sin\left(-\omega_i x + \frac{\pi}{2}\right)  
\end{align}
 
Consequently, both $\omega_i$ and $-\omega_i$ are present in the signal, and we only need to sample the integer values $k_j$ from the half-square $\textbf{K} = [0, B] \times [-B, B]$ (Figure \ref{fig:1a}). The hidden layers are initialized using the scheme from~\cite{sitzmann2019siren}. 

Even though we initialize the first layer of $f$ with a finite and small number of frequencies from the set $\textbf{K}$ (Figure \ref{fig:1a}), the composition of multiple layers generates additional frequencies through non-linear interactions (Figure \ref{fig:1b}). This provides greater expressive power for the model, allowing it to represent a wide range of details.
...
\begin{figure}[!ht]
  \centering
    \begin{subfigure}{0.40\textwidth}
      ...
      \caption{}
      \label{fig:random-mask-data}
    \end{subfigure}
    \begin{subfigure}{0.40\textwidth}
      \includegraphics[width=\linewidth]{img/ch6/extrapolation_randommask.png}
      \caption{}
      \label{fig:random-mask-prediction}
    \end{subfigure}
    \caption{(A) The masked input data, where the black pixels were removed from training. (B) The network reconstruction extrapolated in $[2, 6]^2$.}
  \label{f:masked_recontruction}
\end{figure}

In this experiment, we constructed a mask by randomly selecting a pixel and then excluding all pixels corresponding to it based on the pattern's period. This process was repeated until no further pixels could be sampled, resulting in a mask that covered $1/4$ of the total pixels. We continued to train the image in the interval $[-1, 1]$ but we set the period $P$ to $1$. Figure \ref{fig:random-mask-data} presents the training data, where the black pixels were removed from training, and Figure \ref{fig:random-mask-prediction} shows the learned texture in $[2, 6]^2$.
...
Figure \ref{f:mr-periodic} showcases 5 levels of detail of an M-Net model with 6 stages trained using a Gaussian pyramid of the data in $[-1, 1]^2$ and sampled in $[-2, 2]^2$. The input image has a resolution of $1024^2$ pixels and $8$ bits per color channel in RGB space. 

We explored network stages with a single hidden layer, and heterogeneous width, as experiments have indicated that we need fewer parameters to fit an image of lower resolution and low-frequency details. The initialization follows the strategy described in the Section \ref{sec:integer-initialization}. Table \ref{tab:mnet-architecture} details the band limits of the first layer, the architecture of the hidden layer (number of values received and number of values produced), and the maximum number of epochs that each stage was trained. We used the Adam optimizer with a learning rate of $10^{-3}$.
...
\begin{figure}[!ht]
    \centering
      ...
      \begin{subfigure}{0.40\textwidth}
        ...
        \caption{M-Net reconstruction}
      \end{subfigure}
      \caption{Qualitative comparison between the periodic M-Net reconstruction and the ground image.}
      \label{f:rec_gt}
\end{figure}
...
\includegraphics[width=0.325\textwidth]{img/ch6/dirty/ext5.png}
...
\min \int_{\Omega} (1-\lambda)\norm{{\jac{f}-U}}^2dx \text{ subject to } \lambda(\gt{f}-f)=0 \text{ in } \Omega,
\end{align}

where $\lambda: \Omega \to [0,1]$ is a weight function. When $\lambda \approx 0$, the Jacobian of $f$ should match $U$, while for $\lambda \approx 1$, the condition $\gt{f} = f$ is enforced. Based on this, we propose the following loss function to train the parameters $\theta$ of $f$:

\begin{align}\label{e-blending-no-grad}
\mathscr{L}(\theta)={\int_{\Omega} (1-\lambda)\norm{{\jac{f}-U}}^2dx} + {\int_{\Omega} \lambda\big(\gt{f}-f\big)^2dx}.
\end{align}


This loss function effectively trains $f$ to "seamlessly clone" the primitive of $U$ within the domain $\Omega$ like in \citep{perez2003}. However, since $f$ is periodic, we train it on the torus obtained by identifying opposite edges of $\Omega$. This way, we can "invert the notion of boundary condition", giving the value of the function, the color values, primarily at the interior of the image domain while the boundary is primarily supervised with the gradients. In particular, the mask $\lambda$ depends on the position $x$ and takes lower values near the boundary $\partial \Omega$, ensuring that the Jacobian matching occurs near the edges while the interior enforces color matching. Classical methods do not offer such flexibility.
...
First, we demonstrate that employing periodic activation functions alone is insufficient for learning periodic images with supervision over a restricted domain. Figure \ref{f:siren-comparison} compares image reconstruction and extrapolation results using SIREN's initialization versus our periodic initialization. 

In this experiment, the images were trained on a $512 \times 512$ grid within the region $[-1, 1]^2$ for 500 epochs, and the extrapolations are displayed in the region $[-2, 2]^2$. Both models utilize an identical architecture: three hidden layers of size 256. The key difference lies in the initialization strategy. We randomly select 256 integer pairs of values from the range $[0, 30] \times [-30, 30]$, which were then multiplied by $\frac{2\pi}{P}$, where $P=2$ since the training occurs in $[-1, 1]^2$. In contrast, SIREN follows the initialization described in \cite{sitzmann2019siren}, with $\omega_0 = 30$.
...
Even though the sample image is perfectly tileable, SIREN's reconstruction looks like noise outside the supervised domain. By comparison, our approach successfully captures the underlying periodic pattern. We also applied our Poisson regularization to reconstruct non-tileable patterns with both SIREN and our periodic initialization. Again, Figure \ref{f:comparison_siren_nontileable} shows that, while SIREN's reconstruction results in noise outside the supervised domain, our periodic initialization achieves seamless texture reconstruction.
...
\begin{figure}[!h]
    \centering
      \begin{subfigure}{0.35\textwidth}
        ...
        \caption{Our periodic \\initialization.}
      \end{subfigure}
      \begin{subfigure}{0.35\textwidth}
        \includegraphics[width=\linewidth]{img/ch6/comparisons/siren_grad_border.png}
        \caption{SIREN's \\initialization.}
      \end{subfigure}
    \caption{Non-tileable pattern extrapolation comparison.} 
...
In Chapter \ref{ch:imaging}, we showcased MRNet's capabilities in encoding mip-mapping~\citep{mipmap83} and presenting an anti-aliased version of an image, adapted to the required resolution. Consequently, we can use this method to evaluate a multiresolution network at any coordinate and achieve similar anti-aliasing effects. Figure \ref{f:alias} shows the texture mapping results with and without anti-aliasing treatment utilizing a periodic M-Net with 5 stages.

\begin{figure}[!h]
    \centering
      \begin{subfigure}{0.40\textwidth}
        \includegraphics[width=\linewidth]{img/ch6/alias2.png}
        \caption{No antialiasing; point sampling at the original resolution.}
      \end{subfigure}
      \begin{subfigure}{0.40\textwidth}
        \includegraphics[width=\linewidth]{img/ch6/anti_alias2.png}
        \caption{Antialiasing by sampling the right level of detail.}
      \end{subfigure}
      \caption{Antialiasing in texture mapping using a periodic sinusoidal network.}
    \label{f:alias}
\end{figure}
...
Our current method freezes the weights of the first layer to maintain the period constant during training, working similarly to a Fourier Feature Mapping using only sine functions. An intriguing direction for future research is to make these weights learnable while preserving periodicity, maybe a form of quantized optimization, potentially resulting in more adaptable and higher-performing models.
...
A promising research direction involves the representation and synthesis of quasi-periodic patterns~\citep{quasiRegularTiling} or progressively variant textures, as explored in \cite{variantTextures}. Although the networks we developed are periodic, we believe they can be combined with warping techniques or advanced sampling strategies to generate more complex and diverse patterns.
