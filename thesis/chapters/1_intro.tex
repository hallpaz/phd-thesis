...
% We already have good mathematical models for a variety of media content \red{CITE??}. However, the traditional standards used in industry today are built on top of discrete representations, following the pipeline we described using the paradigm of the four universes. Neural networks can be used to approximate the underlying continuous mathematical model of the media content, bringing the Mathematical Universe and the Implementation Universe closer. One of the advantages of this approach is that from a finite set of samples, it is possible to train a neural network to reconstruct the continuous function and, thus, be ressampled in an arbitrary resolution without the need of interpolation methods, which is more compute efficient.
...
% , following the paradigm of the four universes. 
...
% In this dissertation, we explore how the multiresolution theory can be applied within this framework, developing neural network architectures capable of encoding signals at multiple levels of resolution. Besides helping to solve problems such as zooming and anti-aliasing, a multiresolution representation may be useful for streaming technologies as differrent levels of details can be transferred depending on the speed of the connection and the band capacity. Also exploring multiresolution can be useful for compression, leading to more compact media formats. We also hope that new ways of operating with these neural media will be developing enabling a variaty of applications and we will demonstrante one example of that focused in material textures.
...
% We will show that neural networks can not only approximate the underlying continuous mathematical model of the media content but also provide an efficient representation of its discrete and finite form. 

% It could help to emphasize why a multiresolution neural media framework is important. For example, you could discuss how traditional media encoding faces limitations in terms of scalability, storage, or processing, and how neural networks overcome these.
% The section on encoding media objects using neural networks could benefit from an expanded explanation of why this approach is more efficient or powerful compared to traditional methods. Highlighting specific benefits (e.g., flexibility, adaptability, performance) would strengthen this argument.

% In this work, we focus on encoding media objects using \textbf{artificial neural networks}. 
...
% In this dissertation, we explore fundamental questions that address the intersection of multiresolution analysis, media representation, and neural networks:
...
% - **A comprehensive study on sinusoidal neural network initialization for multiresolution and periodic signal representation.**  
%   We analyze how initialization strategies impact the network’s ability to capture signals across multiple resolutions, laying the groundwork for efficient frequency learning.

% - **Design of a family of neural network architectures for encoding media objects in multiresolution using sinusoidal neural networks.**  
%   This novel approach enables flexible and hierarchical representation of media objects, making it suitable for a wide range of applications that require multiscale encoding.

% - **Development of a flexible framework for training multi-stage neural networks on diverse types of multiresolution data.**  
%   The framework supports different data modalities, allowing for adaptability in training across multiple stages, which is crucial for progressive signal representation.

% - **Application of our architecture to multiresolution image representation and continuous interpolation in space and scale to address anti-aliasing.**  
%   We demonstrate how our model achieves smooth transitions between resolutions, providing a powerful tool for overcoming challenges like aliasing in image processing and rendering tasks.

% - **Introduction of a novel initialization strategy for sinusoidal networks based on the Fourier series to represent periodic signals.**  
%   This ensures that the network remains constrained to the space of periodic functions, despite the complex composition of sines, providing theoretical guarantees for periodic signal encoding.

% - **Efficient encoding of periodic textures using our periodic neural networks and development of a technique based on the Poisson equation to create seamless material textures.**  
%   This contribution addresses a significant challenge in texture generation, enabling seamless transitions and efficient storage, with practical applications in gaming and virtual reality.

% - **Integration of our architecture into the rendering pipeline for textured objects.**  
%   We show that our framework can be incorporated directly into rendering workflows, demonstrating its practical utility for real-time applications like texture mapping and 3D rendering.
