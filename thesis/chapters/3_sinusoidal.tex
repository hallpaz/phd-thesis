...
\red{Note that the reconstructions looks fine as expected.}


Next, we train each network with 4 times less points. The same configuration of networks generated high-oscilating results as shown in Figure XXXX. However, after adjusting the networks width and initialization we reached reasonable approximations of the signals as display in Figure XXXXX.
...
Analysing Figure XXXX, we judge that the reconstruction is good as a lossy compression. In fact, the PSNR value for the shallow network is XXXXX and for the deep one, XXXXXX.The maximim difference is XXX and XXXXX. Since we are not using the sinc function to reconstruct the signal, we were not expecting the reconstruction to be perfect.
